...
- Scenario injection: add/remove agents mid-simulation, trigger server restarts, simulate agent downtime.
...
**Viewer API** (Rust or TypeScript service, WebSocket + REST)
...
- **App screen panel:** Rendered approximation of what this agent's AOE/AOS/AOI would show. For consumers: nearby vendor list with prices and availability. For vendors: incoming payment feed, float status, price card. For exchange agents: portfolio, open positions, bid/ask ladder, recent trades.
...
- **Key inventory panel:** All keys generated by this agent. For each: sequence ID, chain, status (unspent/spent/expired), share amount, creation block, expiration deadline.
...
- **Transaction log:** Global feed, all chains. Columns: timestamp, chain, from → to, amount, fee, block. Filterable by chain, agent, time range, amount range.
...
**Map View** (React, using Leaflet or Mapbox GL)
...
- **Zoomed in:** Individual agent icons with role badges. Vendor open/closed indicator and inventory level. Consumer discovery radius circle. Active transaction arcs (animated lines between participants during in-progress assignments).
...
- Competing exchange agent price discovery: multiple agents trading the same chain converge toward market-clearing spreads.
...
| Sims Phase | Base Dependency | Key Deliverables |
|------------|----------------|------------------|
